...
<main class="m-4">
	{#if $page.status === 500}
		...
		Oops! Something went wrong on our end, but we aren't quite sure what. Please try again later, or
		contact an admin to get it sorted now!
	...
	{:else if $page.status === 403}
		...
		Oops! Your user account is not able to access this part of the system. If you don't think this is
		right, please try again later, or contact an admin to get it sorted now!
	{:else if $page.status === 404}
		<!-- Not found -->
		<h3 class="font-bold text-xl">Error {$page.status} - Not found</h3>
		Oops! The page that you are looking for can't be found, it might be that the item you are looking
		for does not exist anymore. Please try against later, or contact an admin to get it sorted now!
	{:else}
