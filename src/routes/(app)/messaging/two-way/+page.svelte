<script lang="ts">
	...
	function getParentLastMessage(parentId: string): twoWayMessage | undefined {
...
{#if data.isAdmin}
	<div class="flex flex-col gap-2">
		{#each data.parents as parent}
			{@const lastMessage = getParentLastMessage(parent.parentId)}
				...
				>
					<div class="flex flex-col">
						...
						<span class="opacity-50"
							>{lastMessage !== undefined ? lastMessage.messageContent : ''}</span
						>
					</div>
